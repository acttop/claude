(() => {
  ...
  async function handleSend(channel) {
    ...
    // iOS Messages는 여러 번호 + body를 같이 넘기면 일부 기기에서 첫 번째 수신자만
    // 남기고 나머지를 조용히 지워버릴 수 있다(실기기 확인됨). 그래서 문자는 기본이
    // 한 명씩 순서대로이고, 위험을 감수하겠다고 명시적으로 체크한 경우에만 그룹으로 시도한다.
    const smsGroupRisky = $('#mode-sms-group-risky').checked;
    const wantsIndividual = channel === 'sms' ? !smsGroupRisky : $('#mode-individual').checked;
    const individualMode = hasActiveQueue || (targets.length > 1 && wantsIndividual);
